...
const TransactionPage: React.FC = () => {
  ...
  const [transactionEmail, setTransactionEmail] = useState<string | undefined>();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:4000/transaction/${transactionId}?email=${email}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <DashboardPage title="Transaction">
      <div className="transaction-info">
        <span className={`balance_amount ${type}`}>
